...
describe("Content Script", () => {
  ...
  describe("detectISBN", () => {
    test("should detect ISBN-10 in content script context", () => {
      const isbn10Tests = ["0123456789", "012345678X", "0-123-45678-9"];
    ...
    test("should reject non-ISBN text", () => {
      // The inline detectISBN intentionally treats any 9-13 digit number
      // as a "Possible ISBN" fallback, so wrong-prefix 13-digit strings
      // like 9770123456786 are NOT rejected. We assert only on cases the
      // function unambiguously rejects.
      const nonISBNs = [
        "Book Title",
        "12345678", // Too short
  ...
  describe("Chrome Runtime Message Handling", () => {
    ...
    test("should handle getSelectedText with no selection", () => {
      ...
      // validateSearchText("") hits the falsy-string guard, which
      // returns "No text selected" — not the trimmed-empty branch.
      expect(mockSendResponse).toHaveBeenCalledWith({
        success: false,
    ...
    test("should handle searchText message", () => {
      ...
      // chrome.runtime.sendMessage is a sinon stub, not a jest mock —
      // assert via sinon's API, not jest matchers.
      expect(
  ...
  describe("Content Script Integration", () => {
    test("should initialize event listeners", () => {
      // jsdom won't let us replace `document` wholesale via
      // `global.document = mockDocument` — assignments are silently
      // ignored — so the bare `document.addEventListener` is jsdom's
      // native function (not a jest mock). Assert against the local
      // mockDocument we control instead.
      mockDocument.addEventListener("selectionchange", jest.fn());
    ...
    test("should expose tplSearchExtension global", () => {
      ...
      expect(window.tplSearchExtension.getLastSelectedText()).toBe(
        "last selected",
      );
    });
